...
document.addEventListener('click', (event) => {
    ...
    if (key) {
       ...
       // alert(element.InnerHTML);
        ...
        //reactiveBindingData[key] = element.InnerHTML;
        alert(reactiveBindingData[key]);
// Select the button element by its data-bind attribute value
//const button = document.querySelector(`[data-bind="${key}"]`);

// Get the inner text of the button
//const textContent = button.textContent;
//alert (textContent);
//console.log(innerText); // Output: 0


        // Or use innerText if the element is not an input element
        // reactiveBindingData[key] = element.innerText;
    }
});
