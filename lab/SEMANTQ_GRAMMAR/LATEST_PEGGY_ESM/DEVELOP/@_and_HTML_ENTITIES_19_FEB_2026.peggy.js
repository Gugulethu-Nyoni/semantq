...
TextContent
  = text:$(
    ...
    (
      (!('@if(' / '@each(' / '@else' / '@endif' / '@endeach' / '<' / '{') 
        (
          ('&' ('copy;' / 'amp;' / 'lt;' / 'gt;' / 'nbsp;' / 'quot;'))
          / [^@<{]
        )
      )+
    )
...
HTMLEntity
  = "&" entity:(NamedEntity / NumericEntity / HexEntity) ";" {
      const loc = location();
      return {
        type: 'TextNode',
        value: entity,
...
NumericEntity
  = "#" d:$[0-9]+ { return String.fromCodePoint(parseInt(d, 10)); }

HexEntity
  = "#x" h:$[0-9a-fA-F]+ { return String.fromCodePoint(parseInt(h, 16)); }
...
TextWithExpressions
  = parts:(TextPart / MustacheTag / HTMLEntity / ConsequentExpression / CommentBlock / ConsequentContentBlock)+ {
...
LeftHandSideExpression
  = UnaryExpression // <--- ADD THIS LINE HERE
  / CallExpression
  / MemberExpression
  / PrimaryExpression
  ;
...
UnaryExpression
  = op:("!" / "+" / "-" / "~" / "typeof" / "void" / "delete") _ argument:LeftHandSideExpression { // FIXED
...
Condition
  = head:FullJSExpression { return head; }
