...
/** @replaceNodeAtIndex(parent, targetIndex, newNode)
 * you can use this to do the actual replacement 
 * test it first  - worked when tested initally 
 * but we no longer needed so we stopped using/testing :: see TEST BLOCK at the bottom
...
function childrenChecker(nodeToCheck) {
    if (Array.isArray(nodeToCheck)) {
        ...
        for (let key in nodeToCheck) {
            if (findChildren(nodeToCheck[key])) {
        ...
        return findChildren(nodeToCheck);
    }
...
for (let i = 0; i < nodeStack.length; i++) {
  ...
  const result = childrenChecker(nodeToCheck);

  if (result && result.children) {
    const existsIndex = findIndexInChildren(result, targetNode);
