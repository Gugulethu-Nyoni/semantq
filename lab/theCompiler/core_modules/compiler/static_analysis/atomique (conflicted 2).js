 ...
 */
  ...
  // window.incrementer = incrementer;
  // const reRenderFuncCall = `${reRenderFunctionName}(); window.${func} = ${func};`;


    const walk = new Walker();

    console.log(functions, "irst RR",this.reactiveFunctionsRerenders);
  ...
  if (!existsWithValue) {
    const reRenderCallExists = walk.traverseForTypeAndName(this.jsAST, 'CallExpression', reRenderFunctionName, (node, nodeName) => node.callee.name === nodeName).length > 0;

    if (!reRenderCallExists) {
      console.log(func, "reRender not done! adding: ", reRenderFunctionName);

      const functionDeclarationNode = walk.traverseForTypeAndName(this.jsAST, 'FunctionDeclaration', func, (node, nodeName) => node.id.name === nodeName)[0];

      if (functionDeclarationNode) {
        functionDeclarationNode.body.body.push(callerNode);
      } else {
        console.error(`Function ${func} not found in AST.`);
      }

      // Make the function globally available
      const functionGlobaliser = `window.${func} = ${func}`;
      const functionGlobaliserNode = this._jsAstGenerator(functionGlobaliser);
      this.jsAST.body.push(functionGlobaliserNode);
    ...
    console.log(functions, "second RR",this.reactiveFunctionsRerenders);
...
const randomNumber = () => {
  ...
  jsCode +=`let ${rootElementName} = document.createTextNode(${mustacheTagName});`
...
for (let n = 0; n < stackCount; n++) {
  ...
  //console.log("RN", staticNode.reactiveStack); return;
