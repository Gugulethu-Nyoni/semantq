...
function findAllTargetNodes(targetNodeType, ast) {
    const results = [];
            ...
            node.children.forEach((childArray, childKey) => {
                if (Array.isArray(childArray)) {
                    childArray.forEach((child, childIndex) => {
                        _traverse(child, node, childKey, childIndex);
                    });
                } else {
                    _traverse(childArray, node, childKey, null);
                }
            });
        }
    }

    _traverse(ast.html, null, null, null);
    return results;
}
...
const result = findAllTargetNodes(targetNodeType, customSyntaxAST[0]);
