...
class Transpiler extends NodeVisitor {
  constructor(identifiersInFunctions,customSyntaxAST, jsAST, activeBlock,mode) {
    super();
  ...
  transpile() {
    if (this.mode === 1) {
      this.transformReactive(activeBlock);
    } else {
      this.transformStatic(activeBlock);
    }
...
function getActiveNodes(customSyntaxAST, customSyntaxObject, jsAST, nodeStatus, identifiersStatus=null) {
  ...
  if (identifiersStatus === 1) {

    return reactiveIdentifiersOnly; 
  }
...
export default async function transformASTs(jsAST, cssAST, customSyntaxAST, filePath) {
 ...
 // replace with function call to get activeNodes
  const nodeStatus = 1;
 const reactiveIdentifiers = getActiveNodes(customSyntaxAST, customSyntaxObject, jsAST, nodeStatus, 1);

 //console.log("SEE",reactiveIdentifiers);

 let transformedASTs; 

 for (let n = 0; n < reactiveIdentifiers.length; n++) {

  //console.log(reactiveIdentifiers.length);

 const reactiveNode = getActiveNodes(customSyntaxAST, customSyntaxObject, jsAST, nodeStatus);

//console.log("RN",reactiveNode);
const identifiersInFunctions =reactiveNode.identifiersInFunctions;
const activeNode=reactiveStack.activeNode;


  const transpilerStatic = new Transpiler(identifiersInFunctions, customSyntaxObject, jsAST, activeNode, nodeStatus);
 ...
 //console.log("node",reactiveNode); return;
