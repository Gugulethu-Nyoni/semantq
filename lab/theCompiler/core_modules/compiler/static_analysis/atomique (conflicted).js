 ...
 //  if name.type !== EventHandler is not an object and name is not 
  ...
  const elements = walk.deepWalker(customSyntaxAST, anodeType, amatchLogic, areturnType);
   ...
   elements.forEach(element => {
            ...
            //console.log("HERE",element.node.attributes.length);

            for (let i = 0; i < element.node.attributes.length; ++i) {

              const attribute = element.node.attributes[i];

              if (attribute.value[0] && attribute.value[0].type === 'MustacheAttribute') {
        ...
        //console.log("HERE",identifier.node.value[0].type);

/*
   if (identifier.node.value[0] && identifier.node.value[0].type === 'MustacheAttribute' &&  identifier.node.name.type !== 'EventHandler' ) {

      const matchedIdentifier = identifier.node.value[0].name.name;
      attributeIdentifiers.push(matchedIdentifier);


      //console.log("SEE ACTUAL",matchedIdentifier);
    }  
...
/*
   if (identifier.node.value && Array.isArray(identifier.value) && identifier.node.value[0].type  === MustacheAttribute && identifier.node.name.type  && identifier.node.name.type !== EventHandler ) {

      //const matchedIdentifier = identifier.value[0].name.name;

      console.log("SEE",identifier.node.identifier.value[0].name.name);
    }  
*/
...
for (let i = 0; i < aidentifiers.length; ++i) {
    const targetNode = aidentifiers[i].node;
