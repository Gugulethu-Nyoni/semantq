...
class Transpiler extends NodeVisitor {
  constructor(identifiersInFunctions,customSyntaxAST, jsAST, activeStack,mode) {
    super();
  ...
  this.activeStack.forEach(block => {
    if (this.mode === 1) {
      this.transformReactive(block);
    } else {
      this.transformStatic(block);
    }
  });
}
   ...
   transformStatic(block) {
    ...
    console.log("STATIC",block); return;
...
function getIdentifierNodePositions(customSyntaxAST, targetNode) {
  let results = [];

   // console.log(JSON.stringify(customSyntaxAST, null,2));

    const getNodeLocations = new GetNodePositions(customSyntaxAST, targetNode);
    const nodeLocations = getNodeLocations.init();

    //console.log("LOCS", targetNode, nodeLocations.length); 
    ...
    //console.log("LOCATIONS",nodeLocations,"targetNode",targetNode);
...
export default async function transformASTs(jsAST, cssAST, customSyntaxAST, filePath) {
 ...
 //  if name.type !== EventHandler is not an object and name is not 

  // get more identifiers that may not be in the MustacheIdentifier nodeType 
  // e.g. identifiers in attribute values 

  
  let activeStack = [];

//console.log("BABA",identifiers);
  ...
  for (let i = 0; i < identifiers.length; ++i) {
    ...
    //console.log(nodeLocations); return;
    ...
    activeStack.push({
      ...
      // console.log("STACK BEFORE",activeStack);


        // if value[0].type  === MustacheAttribute and name.type !== EventHandler
...
//console.log("SESLA",attributeIdentifiers);
  ...
  for (let i = 0; i < attributeIdentifiers.length; ++i) {
    ...
    //console.log("Please",nodeLocations); return;
  ...
  //console.log("STACK AFTER",activeStack);
 ...
 // console.log(identifiersOnly);

  // Get event handler function calls, if any
  ...
  //console.log("REACTIVE identifiers",identifiersInFunctions);
...
// NOW GET ACTIVE NODES 
  ...
  //console.log("REACTIVE identifiers Only",reactiveIdentifiersOnly);
  //console.log(activeStack);
    const reactiveStack = [];
 ...
 /**@if{reactiveStack}
  * if we have active nodes in the reactive nodes then let's transform them
  */
  
  // Transpile for reactivity and static analysis
  const mode = 1; 
  const transpiler = new Transpiler(identifiersInFunctions, customSyntaxObject, jsAST, reactiveStack, mode);
  transformedASTs = transpiler.getTransformedASTs();
}
...
// Filter activeStack to exclude objects that are also in reactiveStack
const staticStack = activeStack.filter(activeItem => {
  // Check if activeItem exists in reactiveStack
  return !reactiveStack.some(reactiveItem => walk.deepEqual(activeItem, reactiveItem));
});

let survivingStaticNodes = [];

//console.log(JSON.stringify(transformedASTs.transformedCustomSyntaxAST,null,2));

for (let i = 0; i < staticStack.length; ++i) {

  //console.log("TRE",staticStack[i].identifier);

const isSurviving= getIdentifierNodePositions(transformedASTs.transformedCustomSyntaxAST, staticStack[i].activeNode);

if (isSurviving.length > 0) {

survivingStaticNodes.push(staticStack[i]);

}
//console.log("SURVED??? LOOKS",JSON.stringify(isSurviving,null,2));
    
      }



console.log("SN",JSON.stringify(survivingStaticNodes,null,2)); return;
...
//return;
  ...
  const mode = 0;
  const transpilerStatic = new Transpiler(identifiersInFunctions, customSyntaxObject, jsAST, staticStack, mode);
...
/*
let reactiveIdentifiers = [];

for (const item of identifiersInFunctions) {
  const keys = Object.keys(item);
  reactiveIdentifiers.push(keys[0]);
}

//console.log(activeStack);
//console.log(reactiveIdentifiers);

//console.log("BEFORE",activeStack);
function filterOutReactiveIdentifiers(activeStack, reactiveIdentifiers) {
  return activeStack.filter(item => !reactiveIdentifiers.includes(item.identifier));
}

const staticStack= filterOutReactiveIdentifiers(activeStack, reactiveIdentifiers);
//console.log("AFTER",staticStack);
