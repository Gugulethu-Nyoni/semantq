...
class TransformMustacheIdentifierNodes extends NodeVisitor {
 transformMustacheIdentifierNodes(subRootNodeChildren, ast) {
  ...
  /*
  transformMustacheIdentifierNode(identifierNode,ast) {
...
/*
    if (node && node.type && node.type !== "Element" && node.type !== "Fragment") {
...
const subRootNodeChilrdren = findCustomSyntaxNode(ast.html);

console.log(subRootNodeChilrdren); return;
    ...
    // console.log("TYPE",node.type);
      ...
      // console.log(`Parent for ${node.raw}`,parent);
     ...
     //return;
      ...
      if (node.type in visitorsMap && node.type && node.raw !== newLines && node.raw !== whiteSpaces ) {
        //console.log("inside",node); return;
        ...
        if (typeof handlerInstance[transformMethod] === "function") {
          handlerInstance[transformMethod](subRootNodeChilrdren, ast);
        }
        } 
