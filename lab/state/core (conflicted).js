export default class StateOfState {
    constructor(storageKey) {
        ...
        this.bindingData = {};
        this.subscribers = {};
        this.letVariables = this.getObjectDefiners();
        ...
        this.reactiveBindingData = new Proxy(this.bindingData, {
            ...
            set: (target, property, value) => {
                ...
                this.saveStateToStorage(); // Call the saveStateToStorage function
...
notifySubscribers(property, value) {
    ...
    saveStateToStorage() {
        ...
        localStorage.setItem(this.storageKey, JSON.stringify(this.reactiveBindingData));
    }
...
/*
    // Method to search and replace variables in the DOM
    searchVariables(node, letVariables) {
        if (node.nodeType === Node.TEXT_NODE) {
            let content = node.textContent;
            this.letVariables.forEach(variableObj => {
                const regex = new RegExp(`\\{${variableObj.name}\\}`, 'g');
                const matches = content.match(regex);
                if (matches) {
                    this.reactiveBindingData[variableObj.name] = '';
                    matches.forEach(() => {
                        this.reactiveBindingData[variableObj.name] = this.reactiveBindingData[variableObj.name] + ' ' + variableObj.value;
            ...
            node.childNodes.forEach(child => this.searchVariables(child, this.letVariables));
        }
    }
*/
...
// Create an instance of StateManager
 const smqState = new StateOfState('smqState');
