...
function mergeComponents(imports, baseDir, astFile) {
    ...
    // Main AST objects
    const mainJSAST = mainAst[jsKey] || { type: 'JavaScript', content: { body: [] } };
    const mainCSSAST = mainAst[cssKey] || { type: 'CSS', content: { nodes: [] } };
    const mainHtmlAST = mainAst[htmlKey] || { type: 'HTML', content: { html: { children: [] } } };

    // Object to hold merged AST - start with main AST
    let mergedAST = {
        ...mainAst, // Preserve all original properties
        jsAST: mainJSAST,
        cssAST: mainCSSAST,
        [htmlKey]: mainHtmlAST
    };
    ...
    for (const imp of imports) {
        //console.log('Processing import:', imp.specifiers[0]);
        ...
        const componentKey = componentName.toLowerCase(); // e.g. "card"
        ...
        const importHtmlKey = componentKey; // Use lowercase name

        const componentJSAST = componentAST[importJsKey] || { type: 'JavaScript', content: { body: [] } };
        const componentCSSAST = componentAST[importCssKey] || { type: 'CSS', content: { nodes: [] } };
        const componentHtmlAST = componentAST[importHtmlKey] || { type: 'HTML', content: { html: { children: [] } } };

        // Merge JS AST (into main jsAST)
        mergedAST.jsAST.content.body = [
            ...mergedAST.jsAST.content.body,
            ...componentJSAST.content.body
        ];

        // Merge CSS AST (into main cssAST)
        mergedAST.cssAST.content.nodes = [
            ...mergedAST.cssAST.content.nodes,
            ...componentCSSAST.content.nodes
        ];

        // Add component HTML AST under its own key
        if (!mergedAST[componentKey]) {
            mergedAST[componentKey] = componentHtmlAST;
        } else {
            // If key already exists, merge the children
            if (componentHtmlAST.content?.html?.children) {
                mergedAST[componentKey].content.html.children = [
                    ...(mergedAST[componentKey].content?.html?.children || []),
                    ...componentHtmlAST.content.html.children
                ];
            }
        }
    }
