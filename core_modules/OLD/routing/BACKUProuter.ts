...
function handlePath(href: string): void {
    ...
    // Check if currentURL ends with '/' and if protocol and port are present
...
/// if the response from fetch is not index page (which is loaded by default if target file is not found) - then continue to render the new route file

    alert(hash);

    console.log("current page:"+hash);
    console.log("index hash:"+indexFileHash);


            if (hash !== indexFileHash ) 
